...
int kmeans(int iterations,
    ...
    thrust::device_vector<double> *centroids_ngpu[16];
    ...
    for (int q = 0; q < n_gpu; q++) {
       ...
       centroids_ngpu[q] = new thrust::device_vector<double>(k * d);
       thrust::fill(centroids_ngpu[q]->begin(), centroids_ngpu[q]->end(),(double)n_gpu);
    ...
    //Modified by Ang Li, Dec-10, 2017.
    //=================================================
    ncclUniqueId id;
    ncclGetUniqueId(&id);
    ncclComm_t* comm = new ncclComm_t[n_gpu];
    ncclGroupStart();
    for(int i=0; i<n_gpu; i++) {
        cudaSetDevice(i);
        NCCLCHECK(ncclCommInitRank(&comm[i],n_gpu,id,i));
    }
    ncclGroupEnd();
    //=================================================
    ...
    for(; i < iterations; i++) {
        ...
        if (n_gpu > 1){
            ...
            for (int j=0; j<n_gpu; j++){
                cudaSetDevice(j);
                NCCLCHECK(ncclAllReduce(thrust::raw_pointer_cast(centroids[j]->data()),thrust::raw_pointer_cast(centroids[j]->data()), k*d, ncclDouble, ncclSum, comm[j], cuda_stream[j]));
            ...
            for (int j=0; j<n_gpu; j++){
                cudaSetDevice(j);
                thrust::transform(centroids[j]->begin(), centroids[j]->end(), centroids_ngpu[j]->begin(), centroids[j]->begin(), thrust::divides<double>());
            }
        }
    ...
    for (int j=0; j<n_gpu; j++){
        cudaSetDevice(j);
        NCCLCHECK(ncclCommDestroy(comm[j]));
    }
    ncclGroupEnd();
    delete[] comm;
    //=================================================
