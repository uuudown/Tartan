...
int elements;
	...
	if(MyRank != Root)
    {
        ...
        //cudaHostRegister(MatrixB, matrixbsize * sizeof(float), 0);
    }


	//Broad casting the Vector to all the nodes from root node
	MPI_Bcast(MatrixB, matrixbsize, MPI_FLOAT, Root, MPI_COMM_WORLD);
    ...
    //cudaHostRegister(MyMatrixA, ScatterSize * ColsNo * sizeof(float) , 0);
    ...
    //cudaHostRegister(MyResultMatrix,elements* sizeof(float), 0);
...
if (pinned ==1){
	...
	CUDA_SAFE_CALL( cudaHostAlloc( (void **)&DeviceMyMatrixA, ScatterSize * ColsNo * sizeof(float), cudaHostAllocDefault ) );
	CUDA_SAFE_CALL( cudaHostAlloc( (void **)&DeviceMatrixB, matrixbsize*sizeof(float), cudaHostAllocDefault ) );
	CUDA_SAFE_CALL( cudaHostAlloc( (void **)&DeviceMyResultVector, elements * sizeof(float), cudaHostAllocDefault ) );

		//Copying the data from host to device
	cudaMemcpyAsync( (void *)DeviceMyMatrixA, (void *)MyMatrixA, ScatterSize * ColsNo * sizeof(float), cudaMemcpyHostToDevice );
	cudaMemcpyAsync( (void *)DeviceMatrixB, (void *)MatrixB,  matrixbsize*sizeof(float), cudaMemcpyHostToDevice );
}
...
cudaSetDevice(MyRank);
	...
	MPI_Gather(MyResultMatrix,elements, MPI_FLOAT, ResultVector, elements, MPI_FLOAT, Root, MPI_COMM_WORLD);
...
int IntializingMatrixVectors(float **MatrixA, float **MatrixB, float **ResultVector, int RowsNo, int ColsNo, int RowsNo2, int ColsNo2)
{
    ...
    //cudaHostRegister(TempMatrixA, RowsNo * ColsNo * sizeof(float), 0);
    //cudaHostRegister(TempMatrixB, RowsNo2 * ColsNo2 * sizeof(float), 0);
    //cudaHostRegister(TempResultVector, RowsNo * ColsNo2 * sizeof(float), 0);
