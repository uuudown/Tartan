...
int main(int argc, char** argv)
{
    ...
    REAL *l_u_send_buffer; cudaMalloc((void**)&l_u_send_buffer, sizeof(REAL)*Nx*Ny*RADIUS);
    REAL *r_u_send_buffer; cudaMalloc((void**)&r_u_send_buffer, sizeof(REAL)*Nx*Ny*RADIUS);
    REAL *l_u_recv_buffer; cudaMalloc((void**)&l_u_recv_buffer, sizeof(REAL)*Nx*Ny*RADIUS);
    REAL *r_u_recv_buffer; cudaMalloc((void**)&r_u_recv_buffer, sizeof(REAL)*Nx*Ny*RADIUS);
    ...
    checkCuda(cudaMemcpy2D(d_s_u, pitch_bytes, h_s_u, sizeof(REAL)*Nx, sizeof(REAL)*Nx, Ny*_NZ, cudaMemcpyHostToDevice));
    ...
    while (t < tEnd)
	{
		...
		// Runge Kutta Step 0
		checkCuda(cudaMemcpy2DAsync(d_s_uo, pitch_bytes, d_s_u, pitch_bytes, dt_size*Nx, Ny*_NZ, cudaMemcpyDeviceToDevice, computeStream));
		...
		for (unsigned int step = 1; step <= 3; step++) // 3 runge kutta steps!!
		{
			...
			if (rank < numberOfProcesses-1)
			{
				...
				checkCuda(cudaMemcpy2DAsync(r_u_send_buffer, dt_size*Nx, d_r_u_send_buffer, pitch_gc_bytes, dt_size*Nx, Ny*RADIUS, cudaMemcpyDeviceToDevice, r_send_stream));
			...
			if (rank > 0)
			{
				...
				checkCuda(cudaMemcpy2DAsync(l_u_send_buffer, dt_size*Nx, d_l_u_send_buffer, pitch_gc_bytes, dt_size*Nx, Ny*RADIUS, cudaMemcpyDeviceToDevice, l_send_stream));
			...
			if (rank < numberOfProcesses-1)
			{
				...
				checkCuda(cudaMemcpy2DAsync(d_r_u_recv_buffer, pitch_gc_bytes, r_u_recv_buffer, dt_size*Nx, dt_size*Nx, Ny*RADIUS, cudaMemcpyDeviceToDevice, r_recv_stream));
			...
			if (rank > 0)
			{
				...
				checkCuda(cudaMemcpy2DAsync(d_l_u_recv_buffer, pitch_gc_bytes, l_u_recv_buffer, dt_size*Nx, dt_size*Nx, Ny*RADIUS, cudaMemcpyDeviceToDevice, l_recv_stream));
	...
	checkCuda(cudaMemcpy2D(h_s_u, dt_size*Nx, d_s_u, pitch_bytes, dt_size*Nx, Ny*_NZ, cudaMemcpyDeviceToHost));
...
*/
    ...
    checkCuda(cudaFree(l_u_send_buffer));
    checkCuda(cudaFree(l_u_recv_buffer));
    checkCuda(cudaFree(r_u_send_buffer));
    checkCuda(cudaFree(r_u_recv_buffer));
