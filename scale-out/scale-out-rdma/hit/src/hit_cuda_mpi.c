...
void setTransposeCudaMpi(void){
    ...
    cudaMalloc((void**)&aux_host1, size);
    cudaMalloc((void**)&aux_host2, size);
...
void transposeXYZ2YZX(float2* u1,int Nx,int Ny,int Nz,int rank,int sizeMpi){
	...
	cudaCheck(cudaMemcpy((float2*)aux_host1,(float2*)u1,size,cudaMemcpyDeviceToDevice),"copy");
	...
	cudaCheck(cudaMemcpy((float2*)AUX,(float2*)aux_host2,size,cudaMemcpyDeviceToDevice),"copy");
...
void transposeYZX2XYZ(float2* u1,int Nx,int Ny,int Nz,int rank,int sizeMpi){
	...
	cudaCheck(cudaMemcpy((float2*)aux_host1,(float2*)AUX,size,cudaMemcpyDeviceToDevice),"copy");
	...
	cudaCheck(cudaMemcpy((float2*)u1,(float2*)aux_host2,size,cudaMemcpyDeviceToDevice),"copy");
